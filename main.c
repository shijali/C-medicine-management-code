...
int i;
//function for auto increment customer
int getcust_id();
...
void main_menu()
{
  char ch;
	...
	do
	{
		...
		//this function is active when medicine qty is less then 10.
		gotoxy(10,40);
		//textcolor(RED+BLINK);
		printf("Press ");
		gotoxy(16,40);
...
//   window(1,1,80,50);
	...
	gotoxy(10,10);
	printf("=> In This Project we Can Add Medicine ,Customer");
	gotoxy(10,12);
	printf("=> We Can Modifed & Delete Existing Record");
	gotoxy(10,14);
	printf("=> We Can Also Search Medicine ,Customer");
	gotoxy(10,16);
...
//		t();  // THIS FUN IS USED FOR DISPLAY DATE & TIME.......
		...
		if(ch=='S')
		{
			...
			fflush(stdin);
			//textcolor(10);
			fprintf(ptrp_r,"%s %s %d %.2f %.2f %d %d %d\n",p_r.medi_id,p_r.medir_name,p_r.qty,p_r.rate,p_r.total,p_r.sDay,p_r.sMonth,p_r.sYear);
			system("cls");
			gotoxy(20,20);
...
void pur_rpt()
{
  ...
  box();
	   ...
	   //textcolor(WHITE);
	   gotoxy(7,10);
	   printf("ID. Medicine Name.     Qty.  Rate.  Total.   Date");
	   gotoxy(7,12);
	   printf("===================================================================");
	   j=14;
